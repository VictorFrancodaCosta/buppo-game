...
export function animateFly(startId, endId, cardKey, cb, initialDeal = false, isToTable = false, isPlayer = false) {
    ...
    if(isPlayer && window.isPlayerCardSkinEquipped?.('metallic_border')) fly.classList.add('card-skin-metallic-border');
...
export function renderTable(key, slotId, isPlayer = false) {
    ...
    if(isPlayer && window.isPlayerCardSkinEquipped?.('metallic_border')) card.classList.add('card-skin-metallic-border');
