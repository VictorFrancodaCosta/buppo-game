...
window.isLevelUpDealing = false; // NOVO: Controla a animação do Level-Up!
...
function cleanupMatchState() {
    ...
    isProcessing = false; window.isLevelUpDealing = false;
...
window.toggleSoundType = function(type) {
    ...
    } else { window.sfxEnabled = document.getElementById('check-sfx').checked; }
    if(window.saveAudioSettings) window.saveAudioSettings();
};

window.playNavSound = function() { 
    if(!window.sfxEnabled) return; let s = audios['sfx-nav']; 
    if(s) { try { if (s.readyState >= 2) s.currentTime = 0; s.play().catch(()=>{}); } catch(e) {} } 
};

let lastHoverTime = 0;
window.playUIHoverSound = function() {
    if(!window.sfxEnabled) return; let now = Date.now(); if (now - lastHoverTime < 50) return; 
...
// CORREÇÃO: Liga/Desliga do Config apontando corretamente
window.toggleConfig = function() {
    ...
    if (overlay.style.display === 'flex') {
        ...
        box.classList.remove('config-pop-out');
        box.classList.add('config-pop-in');
    }
};
...
window.selectDeck = function(deckType) {
    if(window.sfxEnabled && audios['sfx-deck-select']) { try { audios['sfx-deck-select'].currentTime = 0; audios['sfx-deck-select'].play().catch(()=>{}); } catch(e){} }
...
window.goToLobby = async function(isAutoLogin = false) {
    ...
    if (!userSnap.exists()) {
        ...
        if(d.settings) {
            ...
            if (!window.musicEnabled && MusicController.currentTrackId && audios[MusicController.currentTrackId]) { audios[MusicController.currentTrackId].pause(); } 
            else if (window.musicEnabled && MusicController.currentTrackId && audios[MusicController.currentTrackId]) { audios[MusicController.currentTrackId].play().catch(()=>{}); }
        }
    }
...
function startGameFlow() {
    ...
    startCinematicLoop(); window.isMatchStarting = true; window.isLevelUpDealing = false;
        ...
        if (matchData.status === 'abandoned') {
            if (matchData.abandonedBy && currentUser && matchData.abandonedBy !== currentUser.uid) {
                ...
                setTimeout(() => {
                    ...
                    if(window.registrarVitoriaOnline) window.registrarVitoriaOnline('pvp'); document.getElementById('end-screen').classList.add('visible'); cleanupMatchState();
                }, 500);
            }
            return; 
        }
        ...
        if (window.gameMode === 'pvp' && window.myRole) {
            ...
            if (enemyData) {
                ...
                if (serverXP.length > localXP.length) {
                    ...
                    for (let i = startIdx; i < serverXP.length; i++) { const newCardKey = serverXP[i]; animateFly('m-deck-container', 'm-xp', newCardKey, () => { triggerXPGlow('m'); }, false, false, false); }
...
onAuthStateChanged(auth, (user) => {
    ...
    else { currentUser = null; window.showScreen('start-screen'); const bg = document.getElementById('game-background'); if(bg) bg.classList.remove('lobby-mode'); const btnTxt = document.getElementById('btn-text'); if(btnTxt) btnTxt.innerText = "LOGIN COM GOOGLE"; MusicController.play('bgm-menu'); }
});

window.googleLogin = async function() {
    window.playNavSound(); const btnText = document.getElementById('btn-text'); btnText.innerText = "CONECTANDO...";
    try { await signInWithPopup(auth, provider); } catch (error) { console.error("Erro no Login:", error); btnText.innerText = "ERRO - TENTE NOVAMENTE"; setTimeout(() => btnText.innerText = "LOGIN COM GOOGLE", 3000); }
};
...
function initGlobalHoverLogic() {
    let lastTarget = null;
    document.body.addEventListener('mouseover', (e) => { const selector = 'button, .circle-btn, #btn-fullscreen, .deck-option, .mini-btn'; const target = e.target.closest(selector); if (target && target !== lastTarget) { lastTarget = target; window.playUIHoverSound(); } else if (!target) { lastTarget = null; } });
}
...
function createLobbyFlares() {
    ...
    for(let i=0; i < 70; i++) {
        let flare = document.createElement('div'); flare.className = 'lobby-flare'; flare.style.left = Math.random() * 100 + '%'; flare.style.top = Math.random() * 100 + '%'; let size = 4 + Math.random() * 18; flare.style.width = size + 'px'; flare.style.height = size + 'px'; flare.style.animationDuration = (3 + Math.random() * 5) + 's'; flare.style.animationDelay = (Math.random() * 4) + 's'; container.appendChild(flare);
    }
}
...
function playSound(key) { 
    if(audios[key] && window.sfxEnabled) { 
        try {
            if (key === 'sfx-levelup') {
...
window.saveAudioSettings = async function() {
    if (!currentUser) return;
    try { const userRef = doc(db, "players", currentUser.uid); await updateDoc(userRef, { settings: { vol: window.masterVol, music: window.musicEnabled, sfx: window.sfxEnabled } }); } catch(e) {}
}
...
function triggerDamageEffect(isPlayer, playAudio = true) { 
    try { 
        ...
        if (isPlayer) { document.body.classList.add('shake-screen-hard'); setTimeout(() => document.body.classList.remove('shake-screen-hard'), 400); if(window.triggerDamageEffect) window.triggerDamageEffect(); let ov = document.getElementById('dmg-overlay-old'); if(ov) { ov.style.opacity = '1'; setTimeout(() => ov.style.opacity = '0', 150); } }
    } catch(e) {} 
}
...
function triggerBlockEffect(isPlayer) { 
    try { 
        ...
        if (!isPlayer) { if(window.triggerBlockEffect) window.triggerBlockEffect(); let ov = document.getElementById('block-overlay'); if(ov) { ov.style.opacity = '1'; setTimeout(() => ov.style.opacity = '0', 200); } document.body.classList.add('shake-screen-hard'); setTimeout(() => document.body.classList.remove('shake-screen-hard'), 200); }
    } catch(e) {} 
}
...
// --- CONTROLE DE ANIMAÇÃO DE COMPRA ---
...
// === NOVO FLUXO DE LEVEL-UP COM EMBARALHAMENTO ===
    ...
    if(u.xp.length >= 5) {
        ...
        setTimeout(() => {
            ...
            processMasteries(u, triggers, () => {
                ...
                // 1. Voar as cartas da mão de volta para o deck
                ...
                setTimeout(() => {
                    // 2. Colocar tudo no Deck e Embaralhar
                    ...
                    if (u.id === 'p') {
                        window.isLevelUpDealing = true; // Impede que o update normal mostre a carta direto
                        updateUI(); 
                        dealLevelUpCards(() => { doneCb(true); });
                    } else {
                        updateUI(); doneCb(true);
                    }
                }, 400); // Tempo do voo para o deck
            });
        }, 1000); 
    } else { doneCb(false); }
}
...
function updateUnit(u) {
    ...
    if(u===player) {
        ...
        u.hand.forEach((k,i)=>{
            ...
            // IMPORTANTE: Mantem opacidade 0 apenas se estiver rolando alguma animação de start ou de levelup
            if(window.isMatchStarting || window.isLevelUpDealing) { c.style.opacity = '0'; } else { c.style.opacity = '1'; }
