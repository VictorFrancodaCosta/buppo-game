// ARQUIVO: js/main.js (FASE 1: MATCHMAKING SEGURO)
...
// Variáveis de Matchmaking (NOVAS)
...
window.selectDeck = function(deckType) {
    ...
    // Se for PvP, vamos implementar na FASE 2. Por enquanto, segue fluxo normal.
    setTimeout(() => {
...
function updateUnit(u) {
    ...
    if(u===player) {
        ...
        u.hand.forEach((k,i)=>{
            ...
            c.onclick=()=>onCardClick(i); // Corrigido para chamar direto
            
            let ttHandler = bindFixedTooltip(c,k);
            c.onmouseenter = (e) => { ttHandler.onmouseenter(e); document.body.classList.add('focus-hand'); if(lethalType) isLethalHover = true; playSound('sfx-hover'); };
...
function onCardClick(index) {
    ...
    playSound('sfx-play'); document.body.classList.remove('focus-hand'); document.getElementById('tooltip-box').style.display = 'none'; isLethalHover = false; 
...
function dealAllInitialCards() {
    ...
    cards.forEach((cardEl, i) => {
        cardEl.classList.add('intro-anim'); cardEl.style.animationDelay = (i * 0.1) + 's'; cardEl.style.opacity = ''; 
    });
...
window.googleLogin = async function() {
    ...
    try { await signInWithPopup(auth, provider); } catch (e) { btnText.innerText = "ERRO"; setTimeout(() => btnText.innerText = "LOGIN", 3000); }
};
