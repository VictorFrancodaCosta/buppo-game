...
function startGameFlow() {
    ...
    updateUI(); // Desenha as cartas invisíveis
    setTimeout(() => { dealAllInitialCards(); }, 100);
}

// =======================
// ANIMAÇÕES E VISUAL (CORRIGIDO)
// =======================

// 1. ANIMAÇÃO INICIAL (MÃO) - POP UP COM BOUNCE
function dealAllInitialCards() {
    ...
    cards.forEach((cardEl, i) => {
        setTimeout(() => {
            // Remove opacidade manual e deixa a classe CSS animar
            cardEl.style.opacity = ''; 
            cardEl.classList.add('pop-in');
            playSound('sfx-hover'); 
            
            // IMPORTANTE: Remove a classe de animação ao terminar para destravar o Hover
            setTimeout(() => { cardEl.classList.remove('pop-in'); }, 550);
...
function animateFly(startRef, endRef, cardKey, callback) {
    ...
    // Configura Posição Inicial (Fixed)
...
function onCardClick(index) {
    ...
    const handEl = document.getElementById('player-hand');
    if (!handEl.children[index]) return; // Proteção contra clique fantasma
...
function playCardFlow(index, pDisarmChoice) {
    isProcessing = true; 
    let cardKey = player.hand[index]; // Pega a chave sem remover ainda
    player.hand.splice(index, 1); // Remove dos dados
    ...
    if(realCardEl) {
        realCardEl.style.opacity = '0'; // Esconde o original instantaneamente
        // O Clone voa de onde estava o original
        animateFly(realCardEl, 'p-slot', cardKey, () => { 
            renderTable(cardKey, 'p-slot'); 
            updateUI(); // Atualiza UI (remove carta da mao visualmente)
        });
    } else {
...
function updateUnit(u) {
    ...
    if(u===player) {
        ...
        u.hand.forEach((k,i)=>{
            ...
            // CORREÇÃO CRÍTICA: Opacidade 0 via inline style para garantir que não pisque antes da animação
            c.style.opacity = '0';
