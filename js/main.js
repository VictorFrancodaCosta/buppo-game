        ...
        if (type === 'OUT') {
            fadeIntervals[id] = setInterval(() => {
                if (audio.volume > 0.05) {
                    ...
                    audio.volume = 0;
                    audio.pause();
                    // Não reseta o tempo do menuMusic aqui para não cortar o flow se voltar
            ...
            // 1. Zera volume para o fade
            audio.volume = 0;
            
            // 2. Define o ponto de partida (START ALEATÓRIO)
            if(id === 'bgm-menu') {
                // Se a música já estava pausada, define novo ponto aleatório
                // Se o duration ainda não carregou, usa 60s como base segura
                const maxTime = audio.duration || 60;
                audio.currentTime = Math.random() * (maxTime - 5); 
                console.log("Música Menu - Início Aleatório em: " + audio.currentTime);
            } else {
                // Batalha começa sempre do início
                audio.currentTime = 0; 
            }
...
function updateLoader() {
    ...
    if(assetsLoaded >= totalAssets) {
        ...
        // Isso garante que o Random Start e Fade funcionem logo de cara
        document.body.addEventListener('click', () => { 
            // Verifica se a batalha NÃO está tocando antes de dar play no menu
            const battleMusic = audios['bgm-loop'];
