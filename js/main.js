        ...
        if (type === 'OUT') {
            fadeIntervals[id] = setInterval(() => {
                if (audio.volume > 0.05) {
                    ...
                    audio.volume = 0;
                    audio.pause();
                    // Só reseta a de batalha. O Menu a gente quer que varie.
            ...
            audio.volume = 0; // Começa mudo para o fade
            
            // --- CORREÇÃO DA MÚSICA ALEATÓRIA ---
            // Se for menu, sorteamos um tempo seguro (entre 20s e 50s)
            if(id === 'bgm-menu') {
                try {
                    const safeDuration = (audio.duration && audio.duration > 60) ? audio.duration - 20 : 60;
                    const randomTime = 20 + Math.random() * (safeDuration - 30);
                    audio.currentTime = randomTime;
                    console.log("Iniciando música do Saguão em: " + parseInt(randomTime) + "s");
                } catch(e) { 
                    audio.currentTime = 30; // Fallback se der erro
                }
            } else {
...
window.transitionToGame = function() {
    ...
    setTimeout(() => {
        try {
            ...
            // 2. Troca visual de tela
            let bg = document.getElementById('game-background');
            if(bg) bg.classList.remove('lobby-mode');
            window.showScreen('game-screen');
            
            // 3. NÃO MEXE NAS CARTAS AQUI.
            // Apenas limpa o visual antigo para garantir
        ...
        // 4. Abre a cortina e CHAMA A LÓGICA DO JOGO
        setTimeout(() => {
            if(transScreen) transScreen.classList.remove('active');
            
            // Inicia o jogo DE VERDADE aqui (cria deck, compra cartas, anima)
...
function startGameFlow() {
    ...
    // 1. Reseta os dados lógicos
    resetUnit(player); 
    resetUnit(monster); 
    turnCount = 1; 
    playerHistory = [];
    
    // 2. Compra as cartas (Lógica)
    drawCardLogic(monster, 6); 
    drawCardLogic(player, 6); 
    
    // 3. Cria os elementos no HTML (Visual)
    updateUI();
    
    // 4. Esconde as cartas criadas para preparar a animação
    const handEl = document.getElementById('player-hand'); 
    if(handEl && handEl.children.length > 0) {
        Array.from(handEl.children).forEach(c => c.style.opacity = '0');
        
        // 5. Inicia a animação de distribuição (Fly)
        setTimeout(() => { dealAllInitialCards(); }, 100);
    } else {
        // Fallback de segurança: Se por algum motivo não criou, tenta de novo
        console.log("Hand empty, retrying updateUI...");
        updateUI();
        setTimeout(() => { dealAllInitialCards(); }, 100);
    }
}
